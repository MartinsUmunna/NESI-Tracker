...
const DistributionRevenueCollected = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/Yearly-Revenue-Collected');
  ...
  const processData = (isAnnualView, selectedYear = null) => {
    if (isAnnualView) {
      const annualData = years.map((year) => {
        ...
        const total = yearData.reduce((sum, item) => sum + item.RevenueCollected, 0);
        return {
          year,
      ...
      const monthlyData = monthOrder.map((month) => {
        ...
        return {
          month,
          value: monthData ? Math.round(monthData.RevenueCollected) : 0,
        };
      });
    ...
    <EnergyComparisonAllStatesDashboardWidgetCard
      title={
        <Box display="flex" alignItems="center" justifyContent="space-between" width="100%">
          <Typography variant="h6">Revenue Collected</Typography>
