...
const GencoInvoicetoNBET_Table = () => {
  ...
  const totals = fields.reduce((acc, field) => {
    // Assuming the individual values are already in billions
    acc[field] = rows.reduce((sum, row) => sum + (parseFloat(row[field]) || 0), 0);
    return acc;
  }, {});
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2, p: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6">Genco Invoice to NBET</Typography>
          <Box>
            <FormControlLabel
...
export default GencoInvoicetoNBET_Table;