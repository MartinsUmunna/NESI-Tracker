...
const Compare = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setFetchError(null);
      try {
        // ---- We fetch from the existing endpoints and from our new "Customers" endpoint as well ----
        ...
        const data5 = await fetchMetricsDataCustomers(); // NEW
        ...
        const mergedData = {
          ...
          // ---- MERGE THE NEW CUSTOMER METRICS ----
  ...
  const availableYears = useMemo(() => {
    ...
    selectedMetrics.forEach((metric) => {
      ...
      // ---- NEW METRICS FROM CUSTOMERS (system_collapses, disco_metering_status, total_customer_population, service_band_customers) ----
  ...
  useEffect(() => {
    if (viewMode === 'monthly') {
      const monthlySupported = new Set(['forex', 'transmission_loss', 'capacity', 'service_band_customers']);
  ...
  if (loading) {
    return (
      <PageContainer title="Compare Metrics" description="">
        <BlankCard>
  ...
  return (
    <PageContainer title="Compare Metrics" description="Compare different metrics across your dashboard">
      <BlankCard>
        <Box sx={{ p: 3, height: 'calc(85vh)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
            <Typography variant="h4">Metric Comparison Tool</Typography>
            <Stack direction="row" spacing={2} alignItems="center">
              <FormControlLabel control={<Switch size="small" checked={showDataLabels} onChange={(e) => setShowDataLabels(e.target.checked)} />} label="Data Labels" />
              ...
              {viewMode === 'yearly' && availableYears.length > 0 && (
                <Stack direction="row" spacing={1}>
                  <Select value={yearRange.startYear || ''} onChange={(e) => setYearRange((prev) => ({ ...prev, startYear: e.target.value }))} size="small" sx={{ minWidth: 100 }}>
                  ...
                  <Select value={yearRange.endYear || ''} onChange={(e) => setYearRange((prev) => ({ ...prev, endYear: e.target.value }))} size="small" sx={{ minWidth: 100 }}>
                    {availableYears.map((y) => (
                      <MenuItem key={y} value={y}>
                        {y}
                      </MenuItem>
              ...
              {viewMode === 'monthly' && availableYears.length > 0 && (
                <Select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} size="small" sx={{ minWidth: 100 }}>
                  {availableYears.map((y) => (
                    <MenuItem key={y} value={y}>
                      {y}
                    </MenuItem>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" alignItems="center">
              {selectedMetrics.map((metricKey) => (
                <Box
                  key={metricKey}
                  ref={(el) => {
                    if (el && !popoverAnchors[metricKey]) {
                      setPopoverAnchors(prev => ({ ...prev, [metricKey]: el }));
                    }
                  }}
                >
                  <Chip
          ...
          <Box sx={{ flexGrow: 1, position: 'relative', minHeight: 400, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
